        ...
        if (this.repos.length > 0 || this.allReposFlag) {
          try {
            http.get(`https://api.github.com/users/${this.username}/repos`).then((reposData) => {
              ...
              if (this.allReposFlag) {
                ...
                this.repos.forEach(function(userAddedRepos) {
                  reposData.forEach(function(userAllRepos) {
                    if (userAddedRepos && userAddedRepos.toLowerCase().trim() === userAllRepos.name.toLowerCase().trim()) {
              ...
              console.log("Matching repositories:", reposFound); // Debug: Check matching repos

              if (reposFound.length > 0) {
                let cardContainer = this.cardElem.querySelector('.github-card-container');
                if (cardContainer) {
                  cardContainer.innerHTML += `
                    <span id='github-card-repo-headline' style='font-size:9pt;color:#777;font-weight:bold;text-align:center'>
                      <center>Repositories</center>
                    </span>
                    <div class='github-card-repos' id='github-card-repos'></div>
                  `;
                }

                reposFound.forEach(function(card, i) {
                  if (!card.language) {
                    card.language = 'Unknown'; // Default value if language is null
                  }
                  var div = document.createElement('div');
                  div.id = 'github-card-repo' + (i + 1);
                  div.innerHTML = `
                    <a class='github-card-repo-headline' href="${card.html_url}">
                      <b>${card.name}</b>
                    </a><br>
                    <span class='github-card-repo-desc'>${card.description}</span><br>
                    <span class='gc-lang' style='font-size:8pt;'>&#9733; ${card.language}</span>
                  `;
